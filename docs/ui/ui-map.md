...
- `結果確定・レポート` は上段を `既存レポートの選択` / `新規レポート生成` / `提出状況サマリー` に分割し、提出状況テーブルの各行から `提出状況` タブへ遷移できる。
- `結果確定・レポート` は `entityごとの順位一覧 / 公平性 / 発表準備` の3タブ構成とし、`analysis` は独立タブを廃止して `公平性` 内へ統合する。
- `entityごとの順位一覧` では `集計区分` のセグメントを小枠で表示し、その下に対応テーブル（差分比較選択付き）を配置する。
- `発表準備` は `発表準備サマリ` と `発表境界` を廃止し、スライド設定と表彰出力（コピー/CSV）に集中する。
- `発表準備` のスライドは初期表示で開き、右上の `フルスクリーンで表示` からそのまま発表できる。
- レポート画面の主要操作は `utab:admin-report-metric`（CustomEvent）として発火し、`tab_enter/tab_leave/cta_click/export_complete` を計測する。
