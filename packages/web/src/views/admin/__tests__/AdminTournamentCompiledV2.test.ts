...
describe('AdminTournamentCompiled V2', () => {
  ...
  it('exports speaker-level ballot details by round and all-round results with voter provenance', () => {
    const source = load('src/views/admin/AdminTournamentCompiled.vue')
    const messages = load('src/i18n/messages.ts')
    expect(source).toContain('detailedResultsExportRound')
    expect(source).toContain('detailedResultsExportRows')
    expect(source).toContain('selectedDetailedResultsExportRows')
    expect(source).toContain('snapshotScopedDetailedResultsExportRows')
    expect(source).toContain('compiledExportRoundSet')
    expect(source).toContain("downloadDetailedResultsCsv('selected')")
    expect(source).toContain("downloadDetailedResultsCsv('all')")
    expect(source).toContain('buildDetailedResultsExportRows')
    expect(source).toContain('resolveDetailedExportBallotSide')
    expect(source).toContain('individual-download-grid')
    expect(source).toContain('individual-download-card--all-rounds')
    expect(source).toContain("$t('選択ラウンドの投票詳細CSV')")
    expect(source).toContain("$t('全ラウンドの投票詳細CSV')")
    expect(messages).toContain("'選択ラウンドの投票詳細CSV': 'Selected-round vote details CSV'")
    expect(messages).toContain("'全ラウンドの投票詳細CSV': 'All-round vote details CSV'")
  })

  it('keeps individual CSVs and offers a bundled ZIP download of all available results', () => {
    const source = load('src/views/admin/AdminTournamentCompiled.vue')
    const messages = load('src/i18n/messages.ts')
    expect(source).toContain('bulkExportFiles')
    expect(source).toContain("$t('個別ダウンロード')")
    expect(source).toContain("$t('一括ダウンロード')")
    expect(source).toContain('downloadBulkResultsZip')
    expect(source).toContain('createZip(entries)')
    expect(source).toContain("import { buildTournamentExportFilename } from '@/utils/export-filename'")
    expect(source).toContain("tournamentExportFilename('results.zip')")
    expect(source).toContain("tournamentExportFilename('current_participants.csv')")
    expect(source).toContain("tournamentExportFilename('current_snapshot_round_results.csv')")
    expect(source).toContain("tournamentExportFilename('report_provenance.txt')")
    expect(source).toContain('current_* files can reflect corrections or metadata edits')
    expect(messages).toContain("'結果一式をZIPでダウンロード': 'Download all results (ZIP)'")
  })
