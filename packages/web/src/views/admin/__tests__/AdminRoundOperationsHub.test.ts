...
describe('AdminRoundOperationsHub', () => {
  ...
  it('shows submission overview and venue-based submission table with search', () => {
    ...
    expect(source).toContain("if (item?.type !== 'ballot') return")
    expect(source).toContain('buildSubmissionPreviewEntry(item, index, firstTimestamp, firstSubmissionKey, true)')
    expect(source).toContain('buildSubmissionPreviewEntry(item, index, firstTimestamp, firstSubmissionKey, false)')
    expect(source).toContain('function adjudicatorSubmissionLabel(')
    expect(source).toContain('chairsLabel: adjudicatorSubmissionLabel(row.chairIds, bucket.team)')
    expect(source).toContain("if (isFirstSubmission) return '🏁'")
