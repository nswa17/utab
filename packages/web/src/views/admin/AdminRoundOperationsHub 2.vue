  ...
  <section class="stack">
    ...
    <div v-else class="stack">
      <section class="card stack">
        ...
        <p v-if="sortedRounds.length === 0" class="muted small">{{ $t('No rounds yet.') }}</p>
      ...
      <section class="card stack">
        ...
        <p v-if="selectedRound === null" class="muted small">{{ $t('Select a round.') }}</p>
        <template v-else>
          <p class="muted small flow-caption">
            {{ $t('Proceed from left to right. Run later steps after earlier steps are completed.') }}
          </p>
          ...
          <section v-if="activeTask === 'draw'" class="card soft stack step-card">
            ...
            <p class="muted small">
              {{ $t('Generate and save pairings from draw settings.') }}
            </p>
            <iframe
          ...
          <section v-else-if="activeTask === 'publish'" class="card soft stack step-card">
            <div class="row step-title-row">
              <strong>2. {{ $t('Publish / Lock') }}</strong>
            </div>
            <p v-if="!selectedRoundHasDraw" class="muted small">{{ $t('Generate the draw first.') }}</p>
            <template v-else>
          ...
          <section v-else class="card soft stack step-card">
            ...
            <p v-if="compileDiffBaselineMode === 'compiled' && !compileDiffBaselineCompiledId.trim()" class="muted warning">
              {{ $t('Select a baseline target when comparing with a previous compile.') }}
            </p>
            ...
            <p v-else-if="snapshotIncludesSelectedRound" class="muted small">
              {{ $t('No rows are available for this compiled result.') }}
            </p>
          </section>
        </template>
      </section>
    ...
    >
      <div class="modal card stack" role="dialog" aria-modal="true">
        ...
        <ul class="list compact">
          <li class="list-item">{{ $t('Missing or duplicate submissions may bias the results.') }}</li>
          <li class="list-item">{{ $t('Rows missing submitter IDs may cause omitted or incorrect aggregation.') }}</li>
          <li class="list-item">{{ $t('Mixed submission sources can overwrite each other with latest inputs.') }}</li>
        </ul>
...
const activeTaskHint = computed(() => {
  ...
  if (activeTask.value === 'draw') {
    ...
    if (previousRoundNumbersForDraw.value.length === 0) {
      return t('This round has no prerequisite rounds for compile.')
    }
    return t('対戦生成では最新スナップショットを参照します。')
  }
  if (activeTask.value === 'publish' && !selectedRoundHasDraw.value) {
    return t('Generate the draw first.')
  }
  if (activeTask.value === 'submissions' && !selectedRoundPublished.value) {
    return t('Collect submissions after publication. Publish/lock first.')
  }
  if (activeTask.value === 'compile') {
    ...
    return t('Run compile after collecting submissions.')
  }
  return ''
})
...
function roundCurrentStepLabel(roundNumber: number) {
  ...
  if (nextTask === 'publish') return `2. ${t('Publish / Lock')}`
...
async function refresh() {
  ...
  try {
    ...
    actionError.value = err?.response?.data?.errors?.[0]?.message ?? t('Failed to load data.')
  } finally {
...
async function runCompileWithSource(source: 'submissions' | 'raw') {
  ...
  if (!selectedRoundPublished.value) {
    actionError.value = t('Collect submissions after publication. Publish/lock first.')
    return
  }
  if (source === 'submissions' && shouldBlockSubmissionCompile.value) {
    actionError.value =
      selectedRoundBallotGapWarning.value ||
      t('Cannot run compile because selected round ballots are incomplete.')
    return
  }
  ...
  if (!result) {
    actionError.value = compiledStore.error ?? t('Compilation failed.')
    return
  }
  compileMessage.value = t('Compilation completed.')
...
async function saveDrawPublication(): Promise<boolean> {
  ...
  try {
    ...
    if (!saved) {
      actionError.value = drawsStore.error ?? t('Failed to save publication settings.')
      return false
    }
    publishMessage.value = t('Publication status updated.')
