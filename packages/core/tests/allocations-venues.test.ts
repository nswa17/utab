...
describe('allocations/venues', () => {
  it('assigns lower-priority-number venues first when not shuffled', () => {
    const venues = [
    ...
    expect(venuesAssigned).toEqual([20, 10])
  })

  it('uses venue id as a stable tie-breaker for equal priority', () => {
    const venues = [
      { id: 20, details: [{ r: 1, available: true, priority: 1 }] },
      { id: 10, details: [{ r: 1, available: true, priority: 1 }] },
    ]
    const draw = {
      r: 1,
      allocation: [
  ...
  it('uses seeded shuffle when shuffle is true', () => {
    ...
    const venuesAssigned = result.allocation.map((s: any) => s.venue)
    const prioritySortedVenues = [venues[1], venues[0]]
    const expected = shuffled.map((_, idx) => prioritySortedVenues[idx]?.id ?? null)
